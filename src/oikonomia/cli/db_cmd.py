...
def _gold_parties(gold_path: Path) -> Iterator[PartyObservation]:
    ...
    The validation path — it isolates the gender/party logic from labeler noise, so
...
def women(
    ...
    ``--source gold`` runs over the human annotations — the honest test of the
    gender/party logic. ``--source corpus`` runs the *rule* labeler over the corpus,
    a noisy lower bound only; the real finding runs the **trained model** as the
    extraction engine (people/parties are open-class — see phase_9).
    """
    s = load_settings(env=env, overrides=set_ or [])  # type: ignore[arg-type]
    if source == "gold":
