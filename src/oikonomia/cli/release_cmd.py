...
def check(
    model: Annotated[str, typer.Argument(help="grammateus | homologia | db")],
) -> None:
...
def push(
    ...
    """Upload a model or dataset to the Hub — licence firewall first, private by default.
    ...
    typer.echo(f"{spec.name} → https://huggingface.co/{'datasets/' if spec.repo_type == 'dataset' else ''}{target} ({'private' if private else 'PUBLIC'}, ~{size:.0f} MB)")
    if dry_run:
